...
def type2str(itype, isubtype):
    res = ""
    if itype == APP_TYPE:
        res += "APP :"
    elif itype == DATA_TYPE:
        res += "DATA:"
    else:
        res+="TYPE_{:x}:".format(itype)

    fsubtype = ""
    if itype in [APP_TYPE, DATA_TYPE]:
        fsubtype = SUBTYPES2STR[itype].get(isubtype, None)

    if not fsubtype:
        fsubtype = " SUBTYPE_{:x}".format(isubtype)
...
class PartitionDefinition(object):
    ...
    def __str__(self):
        ...
        return " {:8} {:10} off=0x{:08x} sz=0x{:08x} ".format(self.name, type2str(self.type,self.subtype), self.offset or -1, self.size or -1)
